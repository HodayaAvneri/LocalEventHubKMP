...
class UpdatePostUseCase(private val dashboardRepository: DashboardRepository) {

    operator fun invoke(updatePost: PostEntity) = flow {
        emit(ApiResult.loading())
        val updatePostResponse = dashboardRepository.updatePost(updatePost)
        emit(ApiResult.success(updatePostResponse))
    }.catch {
        println(it.message)

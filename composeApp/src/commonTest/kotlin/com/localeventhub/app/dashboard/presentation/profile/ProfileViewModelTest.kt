...
class ProfileViewModelTest : KoinTest {
    ...
    @Test
    fun `should update user if id and name not null`() = runTest {
        // arrange
        profileViewModel.userId = "1"
        profileViewModel.setName("Test")
        // act
        profileViewModel.validateAndUpdate()

        // assert
        profileViewModel.updateProfileResponse.test {
    ...
    @Test
    fun `should not update user if id and name is null`() = runTest {
        // arrange
        ...
        profileViewModel.updateProfileResponse.test {
            skipItems(2) // To skip initial null and loading
            val result = awaitItem()
            assertEquals(ApiResult.success(UpdateResponse("Failed")), result)
        }
    }
